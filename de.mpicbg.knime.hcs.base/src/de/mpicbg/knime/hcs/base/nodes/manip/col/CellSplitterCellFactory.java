...
class CellSplitterCellFactory implements CellFactory {
    ...
    public DataColumnSpec[] getColumnSpecs() {
        ...
        if (m_outSpecs == null) {
            ...
            String[] splitHeader = selColName.split("::");
            for (int col = 0; col < colNum; col++) {
                String colName = splitHeader[col];
     ...
     * splitted values and of which type the new resulting column have to be. <br> If the "guess" flag in the settings
     * object is NOT set, it returns the column number entered by the user and string type for all columns. Otherwise it
     * runs once through the entire table, splits the value of the selected column, stores the maximum number of parts
     * received, and tries to convert each part into an int (first), then into a double, and if both fails it sets
     * string type for the corresponding column.
     *

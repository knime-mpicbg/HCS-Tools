 ...
 */
public class ZPrimes extends AbstractNodeModel {
    ...
    protected BufferedDataTable[] execute(BufferedDataTable[] inData, ExecutionContext exec) throws Exception {
        ...
        for (DataRow dataRow : barcodeTable) {
            ...
            logger.info("Calculating z-prime for plate '" + barcode + "'");
            ...
            for (String readout : readouts) {
                ...
                double posCtrlSD = beRobust ? mad(posCtrlWells, readoutAttribute) : stdDev(posCtrlWells, readoutAttribute);
                double negCtrlMean = beRobust ? median(negCtrlWells, readoutAttribute) : mean(negCtrlWells, readoutAttribute);
                double negCtrlSD = beRobust ? mad(negCtrlWells, readoutAttribute) : stdDev(negCtrlWells, readoutAttribute);

                double zPrime = 1 - 3 * ((posCtrlSD + negCtrlSD) / Math.abs(posCtrlMean - negCtrlMean));

                Attribute zprimesAttribute = new Attribute(readout, DoubleCell.TYPE);
                updateCache.add(dataRow, zprimesAttribute, isValidNumber(zPrime) ? DataType.getMissingCell() : new DoubleCell(zPrime));
            }
    ...
    protected DataTableSpec[] configure(DataTableSpec[] inSpecs) throws InvalidSettingsException {
        ...
        int specCounter = 0;
        for (String zprimeAttributeName : readouts) {
            allColSpecs[3 + specCounter++] = new DataColumnSpecCreator(zprimeAttributeName, DoubleCell.TYPE).createSpec();
        }
